...
@st.cache_data
def load_template():
    d_data = st.session_state["params"].get("data", {})
    
    file_name = d_data["file_name"]
    #st.markdown("Loading Article template...--ai_blog_template.json> "+file_name + ".json")
    file_name_path = file_name + ".json"
...
def main():
    ...
    d_data = st.session_state["params"].get("data", {})
    page_title = d_data["page_title"]
    template_type_id = d_data["id"]
    #st.json(d_data)
